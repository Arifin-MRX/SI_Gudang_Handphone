...
class PenggunaController extends Controller
{
    ...
    function update(Request $request)
    {
        ...
        if (in_array($request->pegawai_id, [2, 3,5]) && ($pengguna->pegawai_id == 4 || $pengguna->pegawai_id == 5)) {
        ...
        if (in_array($request->pegawai_id, [2, 4,5]) && ($pengguna->pegawai_id == 3|| $pengguna->pegawai_id == 5)) {
            outlet::where('id_pengguna', $pengguna->id)->delete();
            $pengguna->status='Belum Terkonfirmasi';
        }
        if (in_array($request->pegawai_id, [3, 4,5]) && ($pengguna->pegawai_id == 2|| $pengguna->pegawai_id == 5)) {
            $pengguna->status='Belum Terkonfirmasi';
        }
